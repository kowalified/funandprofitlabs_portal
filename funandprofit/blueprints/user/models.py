...
class User(UserMixin, ResourceMixin, db.Model):
    ...
    SCENARIO = OrderedDict([
        ...
        ('31', 'BGP Task 3 Adv 1'),
        ('40', 'IPv6 Task 1'),
        ('41', 'IPv6 Task 1 Adv 1'),
        ('50', 'IPv6 Task 2'),
        ('51', 'IPv6 Task 2 Adv 1'),
        ('60', 'IPv6 Task 3'),
        ('61', 'IPv6 Task 3 Adv 1'),
    ])
    ...
    active = db.Column('is_active', db.Boolean(), nullable=False,
                       server_default='1')
    first_name = db.Column(db.String(34), index=True)
    student_number = db.Column(db.Integer, index=True)
    email = db.Column(db.String(255), unique=True, index=True, nullable=False,
                      server_default='')
    password = db.Column(db.String(128), nullable=False, server_default='funandprofit')

    # Student game properties
    money = db.Column(db.Integer)
    needs_help = db.Column(db.Boolean, server_default='0', index=True)
    help_time = db.Column(db.Time)
    finished_scenario = db.Column(db.Boolean, server_default='0', index=True)
    scenario_time = db.Column(db.Time)
    current_scenario = db.Column(db.Integer, default=1)
    supplement = db.Column(db.Boolean, server_default='0')
    ...
    def initialize_password_reset(cls, identity):
        ...
        ##from funandprofit.blueprints.user.tasks import (
